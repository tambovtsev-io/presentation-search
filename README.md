...
### 2. Валидация
Собран бенчмарк поисковых запросов, проведено тестирование с помощью MLFlow. Предложены несколько вариантов препроцессинга и реранкинга результатов, найден оптимальный - [см. ниже](README.md#4-бенчмарк)
...
## Использование проекта и архитектура
В этой секции я расскажу о том, как пользоваться проектом и о ключевых этапах разработки.
...
- [Ноутбук про описания](notebooks/data_description/count_descriptions.ipynb) - посмотрим, что сгенерировал gpt
...
Промпт для описания можно посмотреть тут - [ссылка](src/chains/prompts:164). Он учитывает несколько факторов:
...
Каждый из элементов описания является целым чанком. Я решил не дробить их долоплнительно. Во-первых так проще. Во-вторых, длина чанков в основном < 700 токенов, и это ок для openai- токенизатора.
...
#### Langsmith
Эту секцию я пока не оформил, потому что в итоге перешел на MLFlow.

## Пайплайны
### Создание базы данных с описаниями
![description pipeline](docs/img/description_pipeline.png)

### Обработка запросов пользователя
![inference pipeline](docs/img/inference_pipeline.png)

**Regex Filter.** Запросы пользователя проходят предобработку перед поиском косинусного расстояния. Из них удаляются стоп слова и фразы, характерные для поиска презентаций. Идея: "к презентации особо по-другому вопрос не задашь...".
